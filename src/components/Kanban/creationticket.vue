...
const form = ref({
  ...
  client_nom: '', client_email: '', client_telephone: '', assigne_id: ''
});
...
const soumettre = async () => {
  ...
  if (!form.value.titre || !form.value.client_nom || !form.value.client_telephone) {
    erreur.value = 'Tous les champs marqués * sont obligatoires'; return;
  }
  const phoneRegex = /^(77|76|78|70|75)[0-9]{7}$/;
  if (!phoneRegex.test(form.value.client_telephone)) {
    erreur.value = 'Numéro invalide (ex: 771234567)'; return;
  }
...
.optionnel { text-transform: none; font-weight: 400; color: #9aa1ad; letter-spacing: 0; }

.input-erreur { border-color: var(--danger) !important; }
.hint-erreur  { font-size: 11px; color: var(--danger); margin-top: 4px; display: block; }
.hint-info    { font-size: 11px; color: var(--ink-soft); margin-top: 4px; display: block; }
...
@media (max-width: 600px) {
  ...
  .creation-ticket { padding: 24px 16px 20px; }
  .grille-2 { grid-template-columns: 1fr; }
  .entete-form h2 { font-size: 18px; }
}
</style>