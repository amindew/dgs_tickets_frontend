  ...
  <div class="admin-page">
    ...
    <div class="admin-header">
      <div>
        ...
        <p class="admin-sous-titre">Créer, modifier et désactiver les comptes</p>
      </div>
    ...
    <div class="filtres-bar">
      <button
        v-for="r in rolesTab" :key="r.val"
        class="filtre-role" :class="{ actif: filtreRole === r.val }"
        @click="filtreRole = r.val"
      >
        {{ r.label }}
        <span class="count">{{ compterRole(r.val) }}</span>
      </button>
    </div>
    ...
    <div class="table-wrap card-surface">
      ...
      <!-- Contenu : desktop ET mobile dans un seul v-else -->
      <template v-else>
        ...
        <table class="table desktop-only">
          ...
          <tbody>
            <tr v-for="u in utilisateursFiltres" :key="u.id" :class="{ inactif: !u.actif }">
              ...
              <td>
                <button v-if="u.id !== authStore.user?.id" class="btn-icone" title="Modifier le rôle" @click="ouvrirModifRole(u)">
        ...
        <div class="cartes-mobile mobile-only">
          <div v-for="u in utilisateursFiltres" :key="u.id" class="carte-user" :class="{ inactif: !u.actif }">
            ...
            <div v-if="u.id !== authStore.user?.id" class="carte-user-actions">
    ...
    <transition name="modal">
      <div v-if="modalOuverte" class="modal-overlay" @click.self="fermerModal">
        <div class="modal card-surface">
          ...
          <div class="champ">
            <label>Email</label>
            <input v-model="form.email" type="email" placeholder="amadou@dgs.sn"
              :class="{ 'input-erreur': emailInvalide }" @blur="validerEmail" />
            <span v-if="emailInvalide" class="hint-erreur">Format invalide</span>
          </div>
          <div class="champ">
            <label>Mot de passe</label>
            <input v-model="form.mot_de_passe" type="password"
              placeholder="Min. 8 caractères, 1 chiffre, 1 majuscule"
              :class="{ 'input-erreur': mdpInvalide }" @blur="validerMdp" />
...
function couleur(u) {
  return palette[u.id.charCodeAt(0) % palette.length]
}
function compterRole(val) {
  return val === 'tous'
    ? utilisateurs.value.length
    : utilisateurs.value.filter(u => u.role === val).length
}
...
function validerMdp() {
  ...
  if (mdp.length < 8) { mdpInvalide.value = true; mdpHint.value = 'Minimum 8 caractères'; return }
  if (!/[A-Z]/.test(mdp)) { mdpInvalide.value = true; mdpHint.value = 'Au moins 1 majuscule'; return }
  if (!/[0-9]/.test(mdp)) { mdpInvalide.value = true; mdpHint.value = 'Au moins 1 chiffre'; return }
...
function ouvrirModifRole(u){ userEnModif.value = u; nouveauRole.value = u.role; modalRoleOuverte.value = true }
...
async function confirmerRole() {
  try {
    ...
    afficherToast('Erreur lors de la modification')
  }
}
...
async function creerUtilisateur() {
  ...
  if (!form.value.nom || !form.value.email || !form.value.mot_de_passe) {
    erreurModal.value = 'Tous les champs sont obligatoires'; return
  }
  ...
  try {
    ...
    fermerModal()
    await charger()
    afficherToast('Utilisateur créé')
  } catch (e) {
    erreurModal.value = e.response?.data?.message || 'Erreur'
  } finally {
...
async function toggleActif(user) {
  try {
    ...
    afficherToast(user.actif ? 'Réactivé' : 'Désactivé')
  } catch (e) {
...
.filtre-role {
  ...
  padding: 6px 12px; font-size: 12px; font-weight: 500; cursor: pointer; color: var(--ink-soft);
}
...
.table tr.inactif td   { opacity: 0.45; }
...
.btn-icone {
  ...
  padding: 5px 7px; cursor: pointer; color: var(--ink-soft);
...
.vide-msg { text-align: center; color: var(--ink-soft); padding: 40px !important; font-size: 13px; }

/* Cartes mobile */
.cartes-mobile { display: flex; flex-direction: column; gap: 1px; }
...
.carte-user-header      { display: flex; justify-content: space-between; align-items: flex-start; gap: 10px; margin-bottom: 10px; }
.carte-badges           { display: flex; flex-direction: column; gap: 4px; align-items: flex-end; }
.carte-user-actions     { display: flex; gap: 8px; }
...
.btn-fermer    { background: none; border: none; cursor: pointer; color: var(--ink-soft); padding: 4px; border-radius: 4px; display: flex; align-items: center; }
.btn-fermer:hover { background: var(--bg); }
...
.message-erreur{ background: var(--danger-soft); color: var(--danger); padding: 10px 14px; border-radius: 8px; font-size: 13px; margin-bottom: 16px; }

.champ        { display: flex; flex-direction: column; gap: 6px; margin-bottom: 14px; }
.champ label  { font-size: 12px; font-weight: 600; color: var(--ink-soft); }
.champ input,
.champ select {
...
.champ input:focus,
.champ select:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(249,115,22,.12); }
