      ...
      <table v-else class="table">
        ...
        <tbody>
          <tr v-for="u in utilisateursFiltres" :key="u.id" :class="{ inactif: !u.actif }">
            <td>
              <div class="user-cell">
                <div class="avatar-md" :style="{ background: couleur(u) }">
                  {{ initiales(u.nom) }}
                </div>
            ...
            <td>
              <!-- Pas de badge statut sur la ligne de l'admin connecté -->
              <span
                v-if="u.id !== authStore.user?.id"
                class="badge-statut"
                :class="u.actif ? 'actif' : 'inactif-badge'"
              >
            ...
            <td>
              <!-- Icône crayon sauf sur sa propre ligne -->
              <button
                v-if="u.id !== authStore.user?.id"
                class="btn-icone"
                title="Modifier le rôle"
                @click="ouvrirModifRole(u)"
              >
            ...
            <td>
              <!-- Bouton désactiver/réactiver sauf sur sa propre ligne -->
              <button
                v-if="u.id !== authStore.user?.id"
                class="btn-action"
                :class="u.actif ? 'desactiver' : 'activer'"
                @click="toggleActif(u)"
              >
        ...
        <div class="modal card-surface">
          ...
          <div class="champ">
            ...
            <input
              v-model="form.email"
              type="email"
              placeholder="amadou@dgs.sn"
              :class="{ 'input-erreur': emailInvalide }"
              @blur="validerEmail"
            />
            <span v-if="emailInvalide" class="hint-erreur">Format invalide (ex: nom@domaine.sn)</span>
          </div>
          <div class="champ">
            ...
            <input
              ...
              :class="{ 'input-erreur': mdpInvalide }"
              @blur="validerMdp"
            />
...
.admin-titre { font-size: 22px; font-weight: 700; color: var(--ink); margin-bottom: 4px; }
...
/* Filtres */
.filtres-bar { display: flex; gap: 8px; margin-bottom: 20px; }
.filtre-role {
  display: flex; align-items: center; gap: 7px;
  background: white; border: 1px solid var(--border);
...
.filtre-role:hover { border-color: var(--border-strong); color: var(--ink); }
.filtre-role.actif { background: var(--navy); border-color: var(--navy); color: white; }
.count { font-size: 11px; background: rgba(0,0,0,0.1); padding: 1px 6px; border-radius: 99px; }
...
.tiret { color: var(--border-strong); font-size: 14px; }
...
.modal-user-nom { font-size: 14px; font-weight: 600; color: var(--ink); margin-bottom: 16px; margin-top: -8px; }
.btn-fermer {
  background: none; border: none; cursor: pointer;
  color: var(--ink-soft); padding: 4px; border-radius: 4px;
  display: flex; align-items: center;
}
.btn-fermer:hover { background: var(--bg); color: var(--ink); }
.modal-footer { display: flex; gap: 10px; justify-content: flex-end; margin-top: 24px; }

/* Validation inputs */
.input-erreur { border-color: var(--danger) !important; }
.hint-erreur { font-size: 11px; color: var(--danger); margin-top: 2px; }
